...
def checkResolution(request):    
    template = loader.get_template('Gmapv2/checkResolution.html')
    allMarkers = GMapMarker.objects.all()
    context = {  
        'allMarkers':allMarkers,        
    }    
...
def editAnnotsCD(request):
    context = {}
    try:
        template = loader.get_template('Gmapv2/editAnnotsCD.html')      
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if(worktype=="checkdams"):
                ...
                print("Year: ",dataOfYear)
                print("District: ",districtName)
                workTypeQueries = Q(work_type__english_name='Cement Concrete Nala Bandh')|\
...
def downloadImage(imgUrl,filepath): #taken from Debanjan's code
    ...
    del response
def save_edited_image_checkdams(request):
    data = {}
    if request.method == 'POST':        
        ...
        district=oldImageName[:firstpos]            
        print(district)
        ...
        delImageName = oldImageName 
        delLocation = location+'deleted/'
        oldfilepath = location + delImageName
        delfilepath=delLocation+delImageName;                
        ...
        imageToBeEdited.centerLatitude = centerLat
        imageToBeEdited.centerLongitude = centerLng
        ...
        if(is_annotated_flag):            
            ...
            if(gmapMarkerId==None):            
                for marker_i in markers:                      
                    ...
                    saveMarker = GMapMarker(source_image=imageToBeEdited,locality=locality,district=district,\
...
def downloadDatasetCDYOLO(request):
    imgs = Image.objects.filter(Q(annotation__class_label="Wall Based Checkdam")|\
                                Q(annotation__class_label="Gate Based Checkdam"),\
                                Q(is_approved=True)).distinct().extra(order_by = ['image_name']);   
    ...
    for img in imgs:
        ...
        for annot in annots:
            ...
            center_y=round(((minCoord["y"]/640)+(diffy/2)),6)
            classnmlab=-1
            if(classnm=="Wall Based Checkdam"):
                classnmlab=CheckdamLabel.WallBasedCD.value;
            if(classnm=="Gate Based Checkdam"):            
                classnmlab=CheckdamLabel.GateBasedCD.value;
        ...
        # with zipfile.ZipFile(zipPath,'a', zipfile.ZIP_DEFLATED) as newzip:
        #     path="static/Gmapv2/images/Checkdams/z19/"
        #     filename=image_name
        #     oldPath=os.path.join(path,filename)
        #     newPath="images/"+filename
        #     newzip.write(oldPath,newPath)
    ...
    response = HttpResponse(open(zipPath, 'rb'), content_type='application/zip')
    response['Content-Disposition'] = 'attachment; filename=%s' % ("CheckdamDatasetYOLO_"+dttm.today().strftime('%Y_%m_%d')+".zip")    
    return response; 
...
def downloadDatasetFPCOCO1(request):
    imgs = Image.objects.filter(Q(annotation__class_label="Wet Farm Pond - Lined")|\
                                Q(annotation__class_label="Wet Farm Pond - Unlined")|\
                                Q(annotation__class_label="Dry Farm Pond - Lined")|\
                                Q(annotation__class_label="Dry Farm Pond - Unlined"),
                                Q(is_approved=True)).distinct().extra(order_by = ['image_name']);
    filename="Dataset-FarmPonds";
    location="static/Gmapv2/zip/coco/"
    newZipFileName=filename+"_0.zip";
    zipPath=os.path.join(location,newZipFileName)        
    lastpos=len(zipPath)-zipPath.rfind('_')
    filepathpattern=zipPath[:-lastpos]
    ...
    for img in imgs:
        ...
        for annot in annots:
            ...
            for coord in pixelCoords:
                pixelCoordsList.append(coord["x"])
                pixelCoordsList.append(coord["y"])            
            minCoord=getMinCoord(pixelCoords)
            maxCoord=getMaxCoord(pixelCoords)
            xmin=minCoord["x"]
            ymin=minCoord["y"]
            ...
            if(classnm=="Dry Farm Pond - Unlined"):            
                category_id=4;
                classnmlab=FarmPondLabel.DryFPUnlined.value;
                #classnmlab=FarmPondLabel.GateBasedCD.value;
    ...
    response = HttpResponse(open(zipPath, 'rb'), content_type='application/zip')
    response['Content-Disposition'] = 'attachment; filename=%s' % ("FarmPondDatasetCOCO_"+dttm.today().strftime('%Y_%m_%d')+".zip")    
    return response;    
...
def downloadDatasetWellsCOCO1(request):
    ...
    image_id=1
    bnd_id=1
    cat = {"supercategory": "none", "id": 1, "name": "Well"}        
    json_dict["categories"].append(cat)    
    for img in imgs:
        row={};        
        image_name=img.image_name                
    ...
    response = HttpResponse(open(zipPath, 'rb'), content_type='application/zip')
    response['Content-Disposition'] = 'attachment; filename=%s' % ("WellDatasetCOCO_"+dttm.today().strftime('%Y_%m_%d')+".zip")    
    return response;    

def downloadDatasetCDCOCO1(request):
    imgs = Image.objects.filter(Q(annotation__class_label="Wall Based Checkdam")|\
                                Q(annotation__class_label="Gate Based Checkdam"),\
                                Q(is_approved=True)).distinct().extra(order_by = ['image_name']);  
    filename="Dataset-Checkdams";
    ...
    image_id=1
    bnd_id=1
    cat = {"supercategory": "none", "id": 1, "name": "Wall Based Checkdam"}        
    json_dict["categories"].append(cat)
    cat = {"supercategory": "none", "id": 2, "name": "Gate Based Checkdam"}
    json_dict["categories"].append(cat)    
    ...
    response = HttpResponse(open(zipPath, 'rb'), content_type='application/zip')
    response['Content-Disposition'] = 'attachment; filename=%s' % ("CheckdamDatasetCOCO_"+dttm.today().strftime('%Y_%m_%d')+".zip")    
    return response;    


def downloadDatasetCDCustom2(request):
    imgs = Image.objects.filter(Q(annotation__class_label="Wall Based Checkdam")|\
                                Q(annotation__class_label="Gate Based Checkdam"),\
                                Q(is_approved=True)).distinct().extra(order_by = ['image_name']);   
...
#### Checkdam dataset recovery ###########
def saveCDfromDStoDB(request):
    template = loader.get_template('Gmapv2/saveCDfromDStoDB.html')
    jsonStr = json.load(open("static/Gmapv2/misc/CD Dataset/dataset.json"));    
    context = {        
        "annotJSON":jsonStr
    }    
    return HttpResponse(template.render(context, request))
